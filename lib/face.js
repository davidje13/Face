(function () {
	...
	function renderLines(pts, {closed = false, pointsAsLines = false} = {}) {
		if (!pts.length) {
			return '';
		}
		if (pts.length === 1) {
	...
	function renderLinesHemi(pts, opts = {}) {
	...
	function buildComponents(dom, root, components, minBackStroke) {
		...
		for (const part in components) {
			...
			if (isBlob) {
				...
				details.elBack = dom.el('g', NS$1);
				details.elFront = dom.el('g', NS$1);
				details.blobs = component.points.map(() => dom.el('use', NS$1)
					.attr('href', '#' + blobID, XLNS)
				);
				const symbol = dom.el('symbol', NS$1).attrs({
					'id': blobID,
					'overflow': 'visible',
				});
				...
				details.elBack = dom.el('path', NS$1).attr('fill-rule', 'evenodd');
				details.elFront = dom.el('path', NS$1).attr('fill-rule', 'evenodd');
			}
	...
	function makeFluffBlob(symbol, dom) {
		const path = dom.el('circle', NS$2).attrs({
			'stroke': '#DDDDDD',
			'stroke-width': 1,
			'fill': '#FFFFFF',
			'cx': '0',
	...
	function flattenToSphere(v, mult) {
		return v.map(clonePt).map((pt$$1) => {
			const dist = Math.sqrt(pt$$1.x * pt$$1.x + pt$$1.y * pt$$1.y + pt$$1.z * pt$$1.z);
			pt$$1.d = (dist - 1) * mult + 1;
			return pt$$1;
		});
	}
